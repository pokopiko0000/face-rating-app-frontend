...
GCS_BUCKET_NAME = "imagen4-faces-imagen-demo-460715"  # ★ あなたのGCSバケット名に書き換えてください
...
METADATA_CSV_PATH = r"C:\Users\j_mar\OneDrive\ドキュメント\face\face0\metadata.csv" # ★ あなたのCSVファイルへの正しいパスに書き換えてください
                                                      # 例: "C:/Users/j_mar/OneDrive/ドキュメント/face/metadata.csv"

# 3. クロップされた顔画像を保存するローカルディレクトリ
LOCAL_CROPPED_FACE_DIR = r"C:\Users\j_mar\OneDrive\ドキュメント\face\face0\cropped_images" # この名前のフォルダがスクリプトと同じ場所に作成されます
...
try:
    ...
    print("モデルファイルパスが正しいか、ultralyticsライブラリが正しくインストールされているか確認してください。")
    exit()
...
try:
    ...
    print("Google Cloud SDKが正しく認証されているか、バケット名が正しいか確認してください。")
    exit()
...
try:
    ...
    required_columns = ["index","country","age","gender","seed","gcs_path"] # 仮の列名
    if not all(col in df_metadata.columns for col in required_columns):
        print(f"エラー: メタデータCSVに必要な列 {required_columns} が含まれていません。")
...
for index, row in tqdm(metadata_to_process.iterrows(), total=len(metadata_to_process), desc="画像処理中"):
    try:
        ...
        gcs_blob_path = os.path.join(GCS_IMAGE_FOLDER_PREFIX, image_filename_on_gcs).replace("\\", "/") # GCSパスは / 区切り
        ...
        print(f"エラー: 画像ID {row.get('image_id', 'N/A')} の処理中に問題が発生しました: {e}")
        import traceback
